...
<script type="text/javascript">
	...
	$(function(){
		$(document).on("click", ".updateReplyForm", function(){
			...
			if($(this).hasClass("active")){
				...
				var html = "<form action='/comment/network/northeast/update' method='post' class='form-horizontal' enctype='multipart/form-data' onsubmit='return commentInsert(this);'>";
	...
	function deleteCheck(num, page) {
		swal({
			...
			if (isConfirm) {
				...
				location.href = "/board/network/northeast/delete?num=" + num + "&page=" + page;
			} else {
	...
	function getReply(obj, board_num, comment_num, isLogin, loginNick, loggedInProfile, board_writer, page){
		if($(obj).hasClass("active")){
			...
			if(isLogin != ""){
				html += "<li>";
				html += "<form action='/comment/network/northeast/insert' class='reply-input' method='post' onsubmit='return commentInsert(this);' enctype='multipart/form-data'>";
			...
			$.ajax({
				url : "/comment/network/northeast/reply/list",
				type : "get",
	...
	function deleteComment(num, board_num, page) {
		swal({
			...
			if (isConfirm) {
				...
				location.href = "/comment/network/northeast/delete?num=" + num + "&board_num=" + board_num + "&page=" + page;
			} else {
	...
	function deleteReply(obj) {
		swal({
			...
			if (isConfirm) {
				swal("Complete", "The comment is deleted.", "success");
				location.href = "/comment/network/northeast/reply/delete?num=" + $(obj).attr("num") + "&board_num=" + $(obj).attr("board_num")  + "&comment_num=" + $(obj).attr("comment_num") + "&page=" + $(obj).attr("page");
			} else {
				swal("Cancel", "The deleting the comment is cancelled.", "error");
			}
		});
					...
					<div class="content-div" style="width: 100%; text-align: right;">
						<c:if test="${boardDTO.writer == loginNick}">
	       				...
	       				<c:if test="${param.search_type ne null }">
							<a class="btn btn-default btn-sm" href="/board/network/northeast/list?page=${param.page }&search_type=${param.search_type}&keyword=${param.keyword}">List</a>
						</c:if>
	       				<c:if test="${param.search_type eq null }">
	       					<a class="btn btn-default btn-sm" href="/board/network/northeast/list?page=${param.page }">List</a>
	       				</c:if>
       				</div>
       				<div class="row" style="margin-top:65px; padding-left: 5px;">
						...
						<c:if test="${isLogin ne null }">
							<form action="/comment/network/northeast/insert" class="comment-input" method="post" onsubmit="return commentInsert(this);" enctype="multipart/form-data">
