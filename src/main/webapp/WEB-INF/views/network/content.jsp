...
<script type="text/javascript">
	...
	$(function(){
		$(document).on("click", ".updateReplyForm", function(){
			...
			if($(this).hasClass("active")){
				...
				var html = "<form action='/comment/network/update' method='post' class='form-horizontal' enctype='multipart/form-data' onsubmit='return commentInsert(this);'>";
	...
	function deleteCheck(num, page) {
		swal({
			...
			if (isConfirm) {
				...
				location.href = "/board/network/delete?num=" + num + "&page=" + page;
			} else {
	...
	function getReply(obj, board_num, comment_num, isLogin, loginNick, loggedInProfile, board_writer, page){
		if($(obj).hasClass("active")){
			...
			if(isLogin != ""){
				html += "<li>";
				html += "<form action='/comment/network/insert' class='reply-input' method='post' onsubmit='return commentInsert(this);' enctype='multipart/form-data'>";
			...
			$.ajax({
				url : "/comment/network/reply/list",
				type : "get",
	...
	function deleteComment(num, board_num, page) {
		swal({
			...
			if (isConfirm) {
				...
				location.href = "/comment/network/delete?num=" + num + "&board_num=" + board_num + "&page=" + page;
			} else {
	...
	function deleteReply(obj) {
		swal({
			...
			if (isConfirm) {
				swal("Complete", "The comment is deleted.", "success");
				location.href = "/comment/network/reply/delete?num=" + $(obj).attr("num") + "&board_num=" + $(obj).attr("board_num")  + "&comment_num=" + $(obj).attr("comment_num") + "&page=" + $(obj).attr("page");
			} else {
				swal("Cancel", "The deleting the comment is cancelled.", "error");
			}
		});
					...
					<div class="content-div" style="width: 100%; text-align: right;">
						<c:if test="${boardDTO.writer == loginNick}">
	       				...
	       				<c:if test="${param.search_type ne null }">
							<a class="btn btn-default btn-sm" href="/board/network/list?page=${param.page }&search_type=${param.search_type}&keyword=${param.keyword}">List</a>
						</c:if>
	       				<c:if test="${param.search_type eq null }">
	       					<a class="btn btn-default btn-sm" href="/board/network/list?page=${param.page }">List</a>
	       				</c:if>
       				</div>
       				<div class="row" style="margin-top:65px; padding-left: 5px;">
						...
						<c:if test="${isLogin ne null }">
							<form action="/comment/network/insert" class="comment-input" method="post" onsubmit="return commentInsert(this);" enctype="multipart/form-data">
		...
		<div class="modal fade" id="update" tabindex="-1" role="dialog" aria-labelledby="contactLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="well well-sm">
					<form class="form-horizontal" action="/board/network/update" method="post" enctype="multipart/form-data" onsubmit="return boardUpdate();">
						<fieldset>
