		...
		<div class="event-row" id="div_event">
			<div class="well">
		        	...
		        	<c:if test="${totalCount > 0 }">
			        	<c:forEach items="${boardList}" var="boardDTO" varStatus="status">
			        		...
			        		<c:if test="${param.search_type eq null}">
			        			<c:set var="href" value="/board/notice/${boardDTO.num }?page=${currentPage}"/>
			        		</c:if>
		        ...
		        <div style="width:100%;" align="center">
					<ul class="pagination">
						<c:if test="${startPage > 1}">
							<c:if test="${param.search_type ne null }">
								<li><a href="/board/notice/list?page=${startPage-1}&search_type=${param.search_type}&keyword=${param.keyword}">&lt;</a></li>
							</c:if>
							<c:if test="${param.search_type eq null }">
								<li><a href="/board/notice/list?page=${startPage - 1}">&lt;</a></li>
							</c:if>
						</c:if>
						...
						<c:if test="${totalPage ne endPage}">
							...
							<c:if test="${param.search_type eq null }">
								<li><a href="/board/notice/list?page=${endPage + 1}">&gt;</a></li>
							</c:if>
						</c:if>
					</ul>
				</div>
				...
				<div style="width: 100%; min-height: 50px;">
					<form action="/board/notice/list" class="form-horizontal search-form col-md-6">
						<div class="col-md-12">
		...
		<div class="modal fade" id="write" tabindex="-1" role="dialog" aria-labelledby="contactLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="well well-sm">
					<form class="form-horizontal" action="/board/notice/insert" method="post" enctype="multipart/form-data" onsubmit="return boardInsert();">
						<fieldset>
